...
def daily_billing_check():
    ...
    for user in gift_users:
        ...
        # Уведомляем
        text = (
            f"🔔 <b>Стабильный интернет</b>\n\n"
            f"Салам аллейкум!\n"
            f"Доступ действует до: <b>{user.end_date}</b>\n"
            f"Для вас всё продлевается автоматически — ничего делать не нужно 😊\n\n"
    ...
    for user in users_to_notify:
        send_message(
            user.telegram_id,
            "🔔 Напоминание: Завтра ваша подписка будет продлена автоматически.",
        )
